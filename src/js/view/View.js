...
import { screens, uiConstants } from './constants';
...
export default class View extends EventObserver {
    ...
    renderStartScreen() {
      this.setFont();
      this.context.textAlign = uiConstants.TEXT_ALIGN_CENTER;
      this.context.textBaseline = uiConstants.TEXT_BASELINE_MIDDLE;
    ...
    renderEndScreen({ score }) {
      ...
      const bestScore = this.notify('getBestScore');
    ...
    renderPanel({
      level, score, lines, nextPiece,
    }) {
      this.context.fillStyle = 'rgba(102, 51, 153, 0.3)';
      this.context.fillRect(this.panelX - 10, 0, this.panelWidth, this.panelHeight);

      this.setFont();
      this.context.textAlign = uiConstants.TEXT_ALIGN_START;
      this.context.textBaseline = uiConstants.TEXT_BASELINE_TOP;
    ...
    setFont() {
      this.context.fillStyle = uiConstants.FONT_COLOR;
      this.context.font = uiConstants.FONT;
    }

    renderBlock(x, y, width, height, color) {
      const gradient = this.context.createRadialGradient(
        x, y, 5, x + width / 2, y + height / 2, 30,
      );
